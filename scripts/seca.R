...
RsquareAdj(rda(spp.pcoa$points ~  elev + H_Al + K + Mg + SLT + AG + N +                                             # Criaçao de objeto contendo valor de referencia
...
plot(rda(spp.pcoa$points ~  elev + H_Al + K + Mg + SLT + AG + N + sol + P + asp1 + asp2 + AF + slop +
                     `elev^2` + `MO^2` + `pH^2` + `K^2` + `CTC^2` + `AG^2` + `SLT^2` + `ARG^2` + `N^2` + `sol^2` + `P^2` + `H_Al^2` + 
                      `V^2` + `asp2^2` + `AF^2` + `slop^2`, data= as.data.frame(envP)), main="Dry RDA")
...
anova.cca(capscale(spp.t ~ Elev +  P + H_Al + K + N + Sol + Silt + `Elev^2` + `N^2` + `AF^2` + `K^2`                     # linhas de codigo para pedir a significancia dos eixos que compoem o modelo
...
RsquareAdj(rda(spp.pcoa$points ~  PCNM_1 + Condition (Elev +  P + H_Al + K + N + Sol + Silt + `Elev^2` + `N^2` + `AF^2` + `K^2`  
...
plot(palm.rda, scaling=1, main=" Dry Triplot RDA spe.hor ~ allfcts - scaling 1 - wa scores")     # basicamente, daqui para baixo o que eu fiz foi elaborar diferentes plots 
...
plot(palm.rda, scaling=2, main="Dry Triplot RDA spe.hor ~ allfcts - scaling 2 - wa scores")
...
plot(palm.rda, scaling=2, display=c("sp","lc","cn"), main="Dry Triplot RDA spe.hor ~ allfcts - scaling 2 - sp+lc+cn ")
arrows(0, 0, palm.sp.sc2[,1], palm.sp.sc2[,2], length= 0, lty=1, col="red")  

plot(palm.rda, scaling=2, display=c("sp","lc"), main="Dry Biplot RDA spe.hor ~ allfcts - scaling 2 - sp+lc")
arrows(0, 0, palm.sp.sc2[,1], palm.sp.sc2[,2], length= 0, lty=1, col="red") 

plot(palm.rda, scaling=2, display=c("sp","cn"), main="Dry Biplot RDA spe.hor ~ allfcts - scaling 2 - sp+cn")
arrows(0, 0, palm.sp.sc2[,1], palm.sp.sc2[,2], length= 0, lty=1, col="red")  

plot(palm.rda, scaling=2, display=c("lc","cn"), main="Dry Biplot RDA spe.hor ~ allfcts - scaling 2 - lc+cn")
